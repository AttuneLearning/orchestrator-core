# Frontend-Dev Model Tiering

Instructions for the `frontend_dev` pull worker (agent_id 3) on how to route work
...
1. **Plan with Opus.** Use Opus to plan **only the current work** — the issue in
   ...
   contract verification (ADR-DEV-001/002 — never invent or paper over a shape),
...
- Contract verification, commit, and gate decisions stay with Opus regardless of
  which tier wrote the code — a cheaper tier must not self-approve its own gate.
- All other frontend-dev boundaries (write scope = `apps/web/` only,
  `packages/contracts` read-only, one-issue-per-branch, never push/merge, direct
  contract consumption, honor per-issue ADRs) remain in force regardless of tier.
